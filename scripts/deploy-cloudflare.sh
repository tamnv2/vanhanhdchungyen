...
for i in $(seq 1 24); do
  # Do not use -f here: we need the JSON body even when health intentionally returns HTTP 503.
...
# Health payload contains only non-sensitive component status/error codes by contract.
echo "Deep health check failed after deploy: $PUBLIC_HOST" >&2
if [[ -n "$last_payload" ]]; then
  echo "Last deep health payload:" >&2
  jq -c . <<<"$last_payload" >&2 2>/dev/null || printf '%s\n' "$last_payload" >&2
else
  echo "Last deep health payload: <empty/unreachable>" >&2
fi
exit 4
